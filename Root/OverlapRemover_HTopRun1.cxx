/*************************************************
 *
 * Interface to Run1-style overlap removal tool 
 *
...
EL::StatusCode OverlapRemover_HTopRun1 :: initialize ()
{
 ...
 if ( m_useCutFlow ) {
  ...
  //RETURN_CHECK( "OverlapRemover_HTopRun1::initialize()", m_overlapRemovalTool->setProperty("OverlapLabel", "overlaps"), ""); // tool will decorate objects with 'overlaps' boolean if they overlap (not possible to customise name atm!)
					     ...
					     SystType syst_type, std::vector<std::string>* sysVec)
{
  ...
  switch ( static_cast<int>(syst_type) )
  {
    ...
    case(0) :  // this is the nominal case
    {
      ...
      if ( m_usePhotons )  RETURN_CHECK("OverlapRemover_HTopRun1::execute()", HelperFunctions::retrieve(inPhotons, m_inContainerName_Photons, m_event, m_store, m_verbose) ,"");
      ...
      RETURN_CHECK( "OverlapRemover_HTopRun1::execute()", m_overlapRemovalTool->setProperty( "OverlapLabel", ORdecor.c_str() ), "Failed to set property OverlapLabel" );
	...
	//
        RETURN_CHECK("OverlapRemover_HTopRun1::execute()", HelperFunctions::retrieve(inElectrons, m_inContainerName_Electrons + systName, m_event, m_store, m_verbose) ,"");
        if ( m_debug ) { Info("execute()",  "inElectrons : %lu, inMuons : %lu, inJets : %lu", inElectrons->size(), inMuons->size(),  inJets->size());  }
	...
	//
        RETURN_CHECK("OverlapRemover_HTopRun1::execute()", HelperFunctions::retrieve(inMuons, m_inContainerName_Muons + systName, m_event, m_store, m_verbose) ,"");
        if ( m_debug ) { Info("execute()",  "inElectrons : %lu, inMuons : %lu, inJets : %lu", inElectrons->size(), inMuons->size(),  inJets->size());  }
    ...
    case (3) : // jet systematics
    {
	 ...
	 //
         RETURN_CHECK("OverlapRemover_HTopRun1::execute()", HelperFunctions::retrieve(inJets, m_inContainerName_Jets + systName, m_event, m_store, m_verbose) ,"");
         if ( m_debug ) { Info("execute()",  "inElectrons : %lu, inMuons : %lu, inJets : %lu", inElectrons->size(), inMuons->size(),  inJets->size());  }

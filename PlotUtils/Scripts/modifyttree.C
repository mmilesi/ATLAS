...
void read_rates(const std::string rr_dir, const std::string fr_dir = "")
{
  ...
  std::string path_R = "/home/mmilesi/PhD/ttH_MultiLeptons/RUN2/PlotUtils/common_ntuple_melbourne/" + rr_dir + "/Rates.root";
  TFile *file_R = TFile::Open(path_R.c_str());
  if ( !file_R->IsOpen() ) {
    SysError("read_rates()", "Failed to open ROOT file with R rate from path: %s . Aborting", path_R.c_str() );
    exit(-1);
  }
  ...
  // ELECTRONS
  //
  std::string histname_el_eta_rr   = ( !CFRateAsRR ) ?  "El_ProbeEta_Real_Rate_expected" : "El_ProbeEta_Fake_Rate_expected";
  std::string histname_el_pt_rr    = ( !CFRateAsRR ) ?  "El_ProbePt_Real_Rate_expected" : "El_ProbePt_Fake_Rate_expected";
  std::string histname_el_eta_r_T  = ( !CFRateAsRR ) ?  "El_ProbeEta_Real_T_expected" : "El_ProbeEta_Fake_Rate_expected";
  std::string histname_el_eta_r_L  = ( !CFRateAsRR ) ?  "El_ProbeEta_Real_L_expected" : "El_ProbeEta_Fake_Rate_expected";
  std::string histname_el_pt_r_T   = ( !CFRateAsRR ) ?  "El_ProbePt_Real_T_expected" : "El_ProbePt_Fake_Rate_expected";
  std::string histname_el_pt_r_L   = ( !CFRateAsRR ) ?  "El_ProbePt_Real_L_expected" : "El_ProbePt_Fake_Rate_expected";

  // get real rate histograms
  //
  TH1D *hist_el_eta_rr   = get_hist( *file_R, histname_el_eta_rr );
  TH1D *hist_el_pt_rr    = get_hist( *file_R, histname_el_pt_rr );
  TH1D *hist_el_eta_r_T  = get_hist( *file_R, histname_el_eta_r_T );
  TH1D *hist_el_eta_r_L  = get_hist( *file_R, histname_el_eta_r_L );
  TH1D *hist_el_pt_r_T   = get_hist( *file_R, histname_el_pt_r_T );
  TH1D *hist_el_pt_r_L   = get_hist( *file_R, histname_el_pt_r_L );

  // MUONS
  //
  std::string histname_mu_eta_rr   = "Mu_ProbeEta_Real_Rate_expected";
  std::string histname_mu_pt_rr    = "Mu_ProbePt_Real_Rate_expected";
  std::string histname_mu_eta_r_T  = "Mu_ProbeEta_Real_T_expected";
  std::string histname_mu_eta_r_L  = "Mu_ProbeEta_Real_L_expected";
  std::string histname_mu_pt_r_T   = "Mu_ProbePt_Real_T_expected";
  std::string histname_mu_pt_r_L   = "Mu_ProbePt_Real_L_expected";

  // get real rate histograms
  //
  TH1D *hist_mu_eta_rr   = get_hist( *file_R, histname_mu_eta_rr );
  TH1D *hist_mu_pt_rr    = get_hist( *file_R, histname_mu_pt_rr );
  TH1D *hist_mu_eta_r_T  = get_hist( *file_R, histname_mu_eta_r_T );
  TH1D *hist_mu_eta_r_L  = get_hist( *file_R, histname_mu_eta_r_L );
  TH1D *hist_mu_pt_r_T   = get_hist( *file_R, histname_mu_pt_r_T );
  TH1D *hist_mu_pt_r_L   = get_hist( *file_R, histname_mu_pt_r_L );
  ...
  std::string path_F = "/home/mmilesi/PhD/ttH_MultiLeptons/RUN2/PlotUtils/common_ntuple_melbourne/" + fake_dir + "/Rates.root";
  TFile *file_F = TFile::Open(path_F.c_str());
  if ( !file_F->IsOpen() ) {
    SysError("read_rates()", "Failed to open ROOT file with F rate from path: %s . Aborting", path_F.c_str() );
    exit(-1);
  }

  // ELECTRONS
  //
  std::string histname_el_eta_fr   = "El_ProbeEta_Fake_Rate_expected";
  std::string histname_el_pt_fr    = "El_ProbePt_Fake_Rate_expected";
  std::string histname_el_eta_f_T  = "El_ProbeEta_Fake_T_expected";
  std::string histname_el_eta_f_L  = "El_ProbeEta_Fake_L_expected";
  std::string histname_el_pt_f_T   = "El_ProbePt_Fake_T_expected";
  std::string histname_el_pt_f_L   = "El_ProbePt_Fake_L_expected";

  // get fake rate histograms
  //
  TH1D *hist_el_eta_fr   = get_hist( *file_F, histname_el_eta_fr );
  TH1D *hist_el_pt_fr    = get_hist( *file_F, histname_el_pt_fr );
  TH1D *hist_el_eta_f_T  = get_hist( *file_F, histname_el_eta_f_T );
  TH1D *hist_el_eta_f_L  = get_hist( *file_F, histname_el_eta_f_L );
  TH1D *hist_el_pt_f_T   = get_hist( *file_F, histname_el_pt_f_T );
  TH1D *hist_el_pt_f_L   = get_hist( *file_F, histname_el_pt_f_L );

  // MUONS
  //
  std::string histname_mu_eta_fr   = "Mu_ProbeEta_Fake_Rate_expected";
  std::string histname_mu_pt_fr    = "Mu_ProbePt_Fake_Rate_expected";
  std::string histname_mu_eta_f_T  = "Mu_ProbeEta_Fake_T_expected";
  std::string histname_mu_eta_f_L  = "Mu_ProbeEta_Fake_L_expected";
  std::string histname_mu_pt_f_T   = "Mu_ProbePt_Fake_T_expected";
  std::string histname_mu_pt_f_L   = "Mu_ProbePt_Fake_L_expected";

  // get fake rate histograms
  //
  TH1D *hist_mu_eta_fr   = get_hist( *file_F, histname_mu_eta_fr );
  TH1D *hist_mu_pt_fr    = get_hist( *file_F, histname_mu_pt_fr );
  TH1D *hist_mu_eta_f_T  = get_hist( *file_F, histname_mu_eta_f_T );
  TH1D *hist_mu_eta_f_L  = get_hist( *file_F, histname_mu_eta_f_L );
  TH1D *hist_mu_pt_f_T   = get_hist( *file_F, histname_mu_pt_f_T );
  TH1D *hist_mu_pt_f_L   = get_hist( *file_F, histname_mu_pt_f_L );
...
void modifyttree(std::string filename = "input.root", std::string  NENTRIES = "ALL", std::string treename= "physics", std::string newfilename = "output.root")
{
  ...
  std::string old_nmuon_name("nmuon");
  std::string old_muon_trkd0sig_name("muon_trkd0sig");
  std::string old_muon_isIsolated_UserDefinedCut_name("muon_isIsolated_UserDefinedCut");
  std::string old_nel_name("nel");
  std::string old_el_LHTight_name("el_LHTight");
  std::string old_el_isIsolated_UserDefinedCut_name("el_isIsolated_UserDefinedCut");
  ...
  Int_t                  nmuon_old;       nmuon_old = -1;
  Int_t                  nel_old;         nel_old = -1;
  std::vector<float>*    muon_trkd0sig_old;       muon_trkd0sig_old = 0;
  std::vector<int>*      muon_isIsolated_UserDefinedCut_old; muon_isIsolated_UserDefinedCut_old = 0;
  std::vector<int>*      el_LHTight_old;  el_LHTight_old = 0;
  std::vector<int>*      el_isIsolated_UserDefinedCut_old; el_isIsolated_UserDefinedCut_old = 0;
  ...
  TBranch	 *b_nmuon_old = 0;  //!
  TBranch	 *b_nel_old = 0;  //!
  TBranch	 *b_muon_trkd0sig_old = 0;  //!
  TBranch	 *b_muon_isIsolated_UserDefinedCut_old = 0;  //!
  TBranch	 *b_el_LHTight_old = 0;  //!
  TBranch	 *b_el_isIsolated_UserDefinedCut_old = 0;  //!
  ...
  oldtree->SetBranchStatus(old_isTT_name.c_str(),0);
  oldtree->SetBranchStatus(old_isTL_name.c_str(),0);
  oldtree->SetBranchStatus(old_isLT_name.c_str(),0);
  oldtree->SetBranchStatus(old_isLL_name.c_str(),0);

  std::string new_MMWeight_name("MMWeight");
  std::string new_isTT_name("isTT");
  std::string new_isTL_name("isTL");
  std::string new_isLT_name("isLT");
  std::string new_isLL_name("isLL");
  std::string new_isTelLmu_name("isTelLmu");
  std::string new_isLelTmu_name("isLelTmu");
  std::string new_isTmuLel_name("isTmuLel");
  std::string new_isLmuTel_name("isLmuTel");
  std::vector<double>*   MMWeight_new;  MMWeight_new = 0;
  Int_t 	         isTT_new; isTT_new = -1;
  Int_t 	         isTL_new; isTL_new = -1;
  Int_t 	         isLT_new; isLT_new = -1;
  Int_t 	         isLL_new; isLL_new = -1;
  Int_t 	         isTelLmu_new; isTelLmu_new = -1;
  Int_t 	         isLelTmu_new; isLelTmu_new = -1;
  Int_t 	         isTmuLel_new; isTmuLel_new = -1;
  Int_t 	         isLmuTel_new; isLmuTel_new = -1;
  ...
  oldtree->SetBranchStatus(old_isTT_name.c_str(),1);
  oldtree->SetBranchStatus(old_isTL_name.c_str(),1);
  oldtree->SetBranchStatus(old_isLT_name.c_str(),1);
  oldtree->SetBranchStatus(old_isLL_name.c_str(),1);
  ...
  oldtree->SetBranchAddress(old_nmuon_name.c_str(), &nmuon_old, &b_nmuon_old);
  oldtree->SetBranchAddress(old_muon_trkd0sig_name.c_str(), &muon_trkd0sig_old, &b_muon_trkd0sig_old);
  oldtree->SetBranchAddress(old_muon_isIsolated_UserDefinedCut_name.c_str(), &muon_isIsolated_UserDefinedCut_old, &b_muon_isIsolated_UserDefinedCut_old);
  oldtree->SetBranchAddress(old_nel_name.c_str(), &nel_old, &b_nel_old);
  oldtree->SetBranchAddress(old_el_LHTight_name.c_str(), &el_LHTight_old, &b_el_LHTight_old);
  oldtree->SetBranchAddress(old_el_isIsolated_UserDefinedCut_name.c_str(), &el_isIsolated_UserDefinedCut_old, &b_el_isIsolated_UserDefinedCut_old);
  ...
  newtree->Branch(new_isTT_name.c_str(), &isTT_new);
  newtree->Branch(new_isTL_name.c_str(), &isTL_new);
  newtree->Branch(new_isLT_name.c_str(), &isLT_new);
  newtree->Branch(new_isLL_name.c_str(), &isLL_new);
  newtree->Branch(new_isTelLmu_name.c_str(),	   &isTelLmu_new);
  newtree->Branch(new_isLelTmu_name.c_str(),	   &isLelTmu_new);
  newtree->Branch(new_isTmuLel_name.c_str(),	   &isTmuLel_new);
  newtree->Branch(new_isLmuTel_name.c_str(),	   &isLmuTel_new);

  // read r/f rates from ROOT histograms
  //
  std::string RR_dir("PLOTS/PLOTS_013/TEST_13F_2/OutputPlots_NonPromptBkgRates_13F");
  //std::string RR_dir("PLOTS/PLOTS_013/TEST_13F_2/OutputPlots_HFBkgRates_13F");

  //std::string RR_dir("PLOTS/PLOTS_018_MMClosure_01/OutputPlots_MMClosureRates_018_OLD_TIGHT_DEF/");

  //std::string RR_dir("OutputPlots_MMClosureRates_13F_ISO_TIGHT_DEF/");


  // when using ch-flip rate as RR (for electrons)
  //std::string RR_dir("PLOTS/PLOTS_013/TEST_13F_2/OutputPlots_ChFlipBkgRates_13F");
  //std::string FR_dir("PLOTS/PLOTS_013/TEST_13F_2/OutputPlots_HFBkgRates_13F");
  ...
  for ( ; i < nentries; i++ ) {
    ...
    if ( !oldtree->GetBranchStatus(old_isTT_name.c_str()) ) {
      oldtree->SetBranchStatus(old_isTT_name.c_str(),1);
    }
    if ( !oldtree->GetBranchStatus(old_isTL_name.c_str()) ) {
      oldtree->SetBranchStatus(old_isTL_name.c_str(),1);
    }
    if  ( !oldtree->GetBranchStatus(old_isLT_name.c_str()) ) {
      oldtree->SetBranchStatus(old_isLT_name.c_str(),1);
    }
    if  ( !oldtree->GetBranchStatus(old_isLL_name.c_str()) ) {
      oldtree->SetBranchStatus(old_isLL_name.c_str(),1);
    }
    ...
    *MMWeight_new = *MMWeight_old;
    ...
    isLT_new = isLT_old;
    isLL_new = isLL_old;
    // Uncomment this block if you wish also to update the T,L definitions
    ...
    // set default values for brand new variables
    //
    isTelLmu_new = 0;
    isLelTmu_new = 0;
    isTmuLel_new = 0;
    ...
    if ( nlep_old == 2 && isSS01_old == 1 ) {
	  ...
	  if ( g_debug2 ) { Info("modifytree()","\t\t EL-EL event ---> change tightness definition...");  }

	  if ( el_isIsolated_UserDefinedCut_old->at(0) == 1 && el_isIsolated_UserDefinedCut_old->at(1) == 1 )	   { isTT_new = 1; }
	  else if ( el_isIsolated_UserDefinedCut_old->at(0) == 1 && el_isIsolated_UserDefinedCut_old->at(1) == 0 ) { isTL_new = 1; }
	  else if ( el_isIsolated_UserDefinedCut_old->at(0) == 0 && el_isIsolated_UserDefinedCut_old->at(1) == 1 ) { isLT_new = 1; }
	  else if ( el_isIsolated_UserDefinedCut_old->at(0) == 0 && el_isIsolated_UserDefinedCut_old->at(1) == 0 ) { isLL_new = 1; }

	}

      } else if ( nmuon_old == 2 ) {

        if ( muon_trkd0sig_old->at(0) < 3.0 && muon_trkd0sig_old->at(1) < 3.0 ) {

	  if ( g_debug2 ) { Info("modifytree()","\t\t MU-MU event ---> change tightness definition...");  }

	  if ( muon_isIsolated_UserDefinedCut_old->at(0) == 1 && muon_isIsolated_UserDefinedCut_old->at(1) == 1 )      { isTT_new = 1; }
	  else if ( muon_isIsolated_UserDefinedCut_old->at(0) == 1 && muon_isIsolated_UserDefinedCut_old->at(1) == 0 ) { isTL_new = 1; }
	  else if ( muon_isIsolated_UserDefinedCut_old->at(0) == 0 && muon_isIsolated_UserDefinedCut_old->at(1) == 1 ) { isLT_new = 1; }
	  else if ( muon_isIsolated_UserDefinedCut_old->at(0) == 0 && muon_isIsolated_UserDefinedCut_old->at(1) == 0 ) { isLL_new = 1; }
        }

      } else if ( nel_old == 1 && nmuon_old == 1 ) {

        if ( el_LHTight_old->at(0) == 1 && muon_trkd0sig_old->at(0) < 3.0 ) {

 	  if ( g_debug2 ) { Info("modifytree()","\t\t OF event ---> change tightness definition...");  }

	  if ( el_isIsolated_UserDefinedCut_old->at(0) == 1 && muon_isIsolated_UserDefinedCut_old->at(0) == 1 )      { isTT_new = 1; }
	  else if ( el_isIsolated_UserDefinedCut_old->at(0) == 1 && muon_isIsolated_UserDefinedCut_old->at(0) == 0 ) { isTL_new = 1; }
	  else if ( el_isIsolated_UserDefinedCut_old->at(0) == 0 && muon_isIsolated_UserDefinedCut_old->at(0) == 1 ) { isLT_new = 1; }
	  else if ( el_isIsolated_UserDefinedCut_old->at(0) == 0 && muon_isIsolated_UserDefinedCut_old->at(0) == 0 ) { isLL_new = 1; }
        }
      }

      if ( g_debug2 ) {
        Info("modifytree()","\t\t OLD isTT = %i \t NEW isTT = %i", isTT_old, isTT_new );
        Info("modifytree()","\t\t OLD isTL = %i \t NEW isTL = %i", isTL_old, isTL_new );
        Info("modifytree()","\t\t OLD isLT = %i \t NEW isLT = %i", isLT_old, isLT_new );
        Info("modifytree()","\t\t OLD isLL = %i \t NEW isLL = %i", isLL_old, isLL_new );
      }

      recomputeMMW( MMWeight_new, isTT_new, isTL_new, isLT_new, isLL_new, *lep_pt_old, *lep_eta_old, *lep_flavour_old );
      */
      ...
      // Store new branches for OF events ( needed for "theta factor" ABCD method)
      //
      if ( nel_old == 1 && nmuon_old == 1 ) {

        if ( isTL_old ) {
          // the tight is an electron, the loose is a muon
          if ( lep_flavour_old->at(0) == 11 )	 { isTelLmu_new = 1; }
          // the tight is a muon, the loose is an electron
          else if ( lep_flavour_old->at(0) == 13 ) { isTmuLel_new = 1; }
        } else if ( isLT_old ) {
          // the loose is an electron, the tight is a muon
          if ( lep_flavour_old->at(0) == 11 )	 { isLelTmu_new = 1; }
          // the loose is a muon, the tight is an electron
          else if ( lep_flavour_old->at(0) == 13 ) { isLmuTel_new = 1; }
        }

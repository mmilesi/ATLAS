...
void LHFitter :: getHists() {
	...
	for ( const auto& sl : m_obs_selection ) {
		...
		if ( m_nBinGroup > 1 ) {
		    ...
		    // Need this trick to create 2D rebinned histogram w/ variable bin size on X,Y axes
		    //
		    // Logic stolen from https://github.com/rootpy/rootpy/blob/master/rootpy/plotting/hist.py#L1407

		    Info("getHists()","Rebinning TH2 w/ variable bin size:");
		    ...
		    TArrayD* sumw2_rebinned = hrebinned->GetSumw2();

		    // Get a few properties of input histogram
		    TAxis *xaxis = hist->GetXaxis();
		    TAxis *yaxis = hist->GetYaxis();
		    int nbinsx = xaxis->GetNbins()+2;
		    int nbinsy = yaxis->GetNbins()+2;
		    TArrayD* sumw2 = hist->GetSumw2();

		    // OK, now fill!
		    int idx, idx_rebinned;
		    ...
		    hrebinned->SetEntries(hist->GetEntries());
            ...
            } // closes check on rebinning

	    m_histograms.push_back(hist);

        } // closes obs_selection loop

    } // Closes flavour_comb loop
...
void  LHFitter :: saveEfficiencies() {
	...
	std::string canvasname(outfilename);
	TCanvas* c = new TCanvas(canvasname.c_str(),"Efficiency",50,50,800,600);
	...
	std::string outcanvasname = canvasname + ".png";
	c->SaveAs(outcanvasname.c_str());
...
int main( int argc, char **argv ) {
    ...
    //const std::string input_path("../OutputPlots_MMRates_LHFit_DLT_25ns_v26_LeptonMVA_Bin_10_15_20_26_30_40_50_90_140_210/");
	...
	double real_ee_new_bins[array_real_bin_size] = {10,15,20,26,30,40,60,90,140,210};
	...
	int array_real_bin_size(10);
	double real_mm_new_bins[array_real_bin_size] = {10,15,20,26,30,40,50,90,140,210};
	...
	int array_real_bin_size(10);
	//double real_of_new_bins[array_real_bin_size] = {10,15,20,26,30,40,60,90,140,210};
	double real_of_new_bins[array_real_bin_size] = {10,15,20,26,30,40,50,90,140,210};

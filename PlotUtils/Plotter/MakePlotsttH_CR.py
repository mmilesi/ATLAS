...
from Plotter.ttH2013_Background import MyCategory, TTHBackgrounds2013
...
inputs = loadSamples(   
                        ...
                        inputdir = '/home/data/ttH/NewReprocessed_Melb13_ttH_v1.1'+selection,
                        samplescsv  = 'Files/samples2013_ttH.csv',#Files containing the processes of interest with their cross sections and other info
...
# A list of cuts
vardb.registerCut( Cut('HFOR',      'top_hfor_type!=4') )
vardb.registerCut( Cut('TrigMatch', 'evtsel_is_sltmatch') )
...
tth2013.luminosity = 20.2814
#tth2013.luminosity = 13.0605
#tth2013.luminosity = 5.83521

tth2013.useZCorrections = True
...
samplenames = { 'Observed':'observed', 'TTBarH':'signal', 'TTBarW':'ttbarwbkg', 'TTBarZ':'ttbarzbkg', 'Top':'topbkg', 'TopCF':'topcfbkg', 'Diboson':'dibosonbkg', 'DibosonCF':'dibosoncfbkg', 'HtoZZ':'htozzbkg', 'Zjets':'zjetsbkg', 'Wjets':'wjetsbkg', 'Prompt':'promptbkg', 'ChargeFlip':'chargeflipbkg', 'FakesFF':'fakesbkg', 'FakesMM':'fakesbkg' }
colors = { 'Observed':kBlack, 'TTBarH':kBlack, 'TTBarW':kRed-4, 'TTBarZ':kRed-7, 'Top':kBlue-3, 'TopCF':kAzure-4, 'Diboson':kOrange, 'DibosonCF':kOrange-3, 'HtoZZ':kTeal+9, 'Zjets':kGreen, 'Wjets':kYellow, 'Prompt':kOrange, 'ChargeFlip':kAzure-4, 'FakesFF':kAzure-9, 'FakesMM':kTeal-9 }

if doStandardPlots:
        if doMM:
                plotbackgrounds = [ 'TTBarW', 'TTBarZ', 'Top', 'TopCF', 'Diboson', 'DibosonCF', 'HtoZZ', 'Zjets', 'FakesMM']
                tth2013.backgrounds = [ 'TTBarW', 'TTBarZ', 'Top', 'TopCF', 'Diboson', 'DibosonCF', 'HtoZZ', 'Zjets', 'FakesMM']#this is the list of backgrounds that will be calculated
        elif doFF:
                plotbackgrounds = [ 'TTBarW', 'TTBarZ', 'Top', 'TopCF', 'Diboson', 'DibosonCF', 'HtoZZ', 'Zjets', 'FakesFF']
                tth2013.backgrounds = [ 'TTBarW', 'TTBarZ', 'Top', 'TopCF', 'Diboson', 'DibosonCF', 'HtoZZ', 'Zjets', 'FakesFF']#this is the list of backgrounds that will be calculated
        else:
                #MC based estimate of fakes
                plotbackgrounds = [ 'TTBarW', 'TTBarZ', 'Top', 'TopCF', 'Diboson', 'DibosonCF', 'HtoZZ', 'Zjets', 'Wjets']
                tth2013.backgrounds = [ 'TTBarW', 'TTBarZ', 'Top', 'TopCF', 'Diboson', 'DibosonCF', 'HtoZZ', 'Zjets', 'Wjets']#this is the list of backgrounds that will be calculated
	if doFourLepSR:
		#no fakes
                plotbackgrounds = [ 'TTBarW', 'TTBarZ', 'Top', 'TopCF', 'Diboson', 'DibosonCF', 'HtoZZ', 'Zjets']
                tth2013.backgrounds = [ 'TTBarW', 'TTBarZ', 'Top', 'TopCF', 'Diboson', 'DibosonCF', 'HtoZZ', 'Zjets']#this is the list of backgrounds that will be calculated

if doElCR or doMuCR:
        #here the fakes are the difference between data and mc
        plotbackgrounds = [ 'TTBarW', 'TTBarZ', 'Top', 'TopCF', 'Diboson', 'DibosonCF', 'HtoZZ', 'Zjets']#, 'Wjets']#The W+jets should be 0 in the truthmatching case and should be a rough estimate of fakes in the other case. Also ttbar should be bigger in the no truth matching case
        tth2013.backgrounds = [ 'TTBarW', 'TTBarZ', 'Top', 'TopCF', 'Diboson', 'DibosonCF', 'HtoZZ', 'Zjets']#, 'Wjets']#this is the list of backgrounds that will be calculated
...
for category in vardb.categorylist:
    ...
    for var in vardb.varlist:
        ...
        if cut:
            dirname = 'output/ttH/v1_1_results'+selection+fakeestimate+'/'  + category.name + ' ' + cut.cutname
        else:
            dirname = 'output/ttH/v1_1_results'+selection+fakeestimate+'/'  + category.name
	if dolog:

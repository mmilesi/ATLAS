...
void LHFitter :: getEducatedGuess( ) {
  ...
  if ( m_flavour == kFlavour::MUON || m_efficiency == kEfficiency::FAKE || m_doRebinning ) {
    if ( m_doRebinning ) {
      std::cout << "" << std::endl;
      Info("getEducatedGuess()","===> using average r,f to determine initial RR, RF, FR, FF yields..." );
      Info("getEducatedGuess()","<r> = %.2f - <f> = %.2f", m_r_init_avg, m_f_init_avg );
      for ( int ibin(1); ibin <= m_nPtBins_Linear; ++ibin ) {
        m_r_init.at(ibin-1) = m_r_init_avg;
        m_f_init.at(ibin-1) = m_f_init_avg;
      }
    } else {
      std::cout << "" << std::endl;
      Info("getEducatedGuess()","===> using average f to determine initial RR, RF, FR, FF yields..." );
...
void  LHFitter :: saveEfficiencies() {
  ...
  if ( m_efficiency == kEfficiency::FAKE ) {
    ...
    TH1D *f_hist = new TH1D( "f_hist", "fake efficiency", m_nPtBins_Linear, 10, 130 );
...
int main( int argc, char **argv ) {
    ...
    real_e.setTagAndProbePath("../OutputPlots_MMRates_25ns_v7_FinalSelection_LHComparison/Rates_NoSub_LHInput_6x6bins/");
    real_e.setInputHistPath("../OutputPlots_MMRates_LHFit_2DPt/");
    real_e.m_doRebinning = true;
    real_e.m_nBinGroup = 2;
    real_e.initialise();
    real_e.fit();
    ...
    real_mu.setTagAndProbePath("../OutputPlots_MMRates_25ns_v7_FinalSelection_LHComparison/Rates_NoSub_LHInput_6x6bins/");
    real_mu.setInputHistPath("../OutputPlots_MMRates_LHFit_2DPt/");
    real_mu.m_doRebinning = true;
    real_mu.m_nBinGroup = 2;
    real_mu.initialise();
    real_mu.fit();
    ...
    fake_e.setVerbosity(LHFitter::kVerbosity::VERBOSE);
    fake_e.setTagAndProbePath("../OutputPlots_MMRates_25ns_v7_FinalSelection_LHComparison/Rates_NoSub_LHInput_6x6bins/");
    fake_e.setInputHistPath("../OutputPlots_MMRates_LHFit_2DPt/");
    fake_e.m_doRebinning = true;
    fake_e.m_nBinGroup = 2;
    fake_e.initialise();
    fake_e.fit();
    ...
    fake_mu.setVerbosity(LHFitter::kVerbosity::VERBOSE);
    fake_mu.setTagAndProbePath("../OutputPlots_MMRates_25ns_v7_FinalSelection_LHComparison/Rates_NoSub_LHInput_6x6bins/");
    fake_mu.setInputHistPath("../OutputPlots_MMRates_LHFit_2DPt/");
    fake_mu.m_doRebinning = true;
    fake_mu.m_nBinGroup = 2;    
    fake_mu.initialise();
    fake_mu.fit();

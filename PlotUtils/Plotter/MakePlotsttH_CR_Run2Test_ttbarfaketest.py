...
from Plotter.BackgroundTools_ttH_ttbarfaketest import loadSamples, Category, Background, Process, VariableDB, Variable, Cut, Systematics, Category
...
from Plotter.ttH2013_Background_Run2Test_ttbarfaketest import MyCategory, TTHBackgrounds2013
...
if doStandardPlots:
        if doMM:
                ...
                tth2013.backgrounds = ['FakesMM']#this is the list of backgrounds that will be calculated
        elif doFF:
                plotbackgrounds = ['FakesFF']
                tth2013.backgrounds = ['FakesFF']#this is the list of backgrounds that will be calculated
        else:
                #MC based estimate of fakes
                plotbackgrounds = [ 'TopCF']
                tth2013.backgrounds = [ 'TopCF']#this is the list of backgrounds that will be calculated
	if doFourLepSR:
		#no fakes
...
if doElCR or doMuCR:
        #I put in the backgrouns Fakes MM even f it does not make senso. I won't do background subtraction of backgrounds at the end to calculate rates
        plotbackgrounds = [ 'FakesMM' ]#, 'Wjets']#The W+jets should be 0 in the truthmatching case and should be a rough estimate of fakes in the other case. Also ttbar should be bigger in the no truth matching case
        tth2013.backgrounds = [ 'FakesMM' ]#, 'Wjets']#this is the list of backgrounds that will be calculated
...
for category in vardb.categorylist:
    ...
    for var in vardb.varlist:
        ...
        if cut:
            dirname = 'output/ttH/Run2Test_ttbarfaketest_results'+selection+fakeestimate+'/'  + category.name + ' ' + cut.cutname
        else:
            dirname = 'output/ttH/Run2Test_ttbarfaketest_results'+selection+fakeestimate+'/'  + category.name
	if dolog:

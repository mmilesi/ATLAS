...
parser.add_argument('--fakeMethod', dest='fakeMethod', action='store', default='MC', type=str,
		    help='the fake estimation method chosen ( MC,MM,FF )')
...
vardb.registerCut( Cut('NJet3L',      '( njets >= 4 || ( njets_mv2c20_Fix70 > 1 && njets == 3 ) )') )
...
# TEST: derive fake rate from OS region: select events where there is at least one non-prompt ( be inclusive in charge flip ) 
#
#vardb.registerCut( Cut('2Lep_NonPromptEvent', '( isMC==0 || ( isMC==1 && ( ( lep_truthType[0] != 6 && lep_truthType[0] != 2 ) || ( lep_truthType[1] != 6 && lep_truthType[1] != 2 ) ) ) )') )
...
if doStandardPlots :
    ...
    #
    #vardb.registerVar( Variable(shortname = 'pT_Z',    	   latexname = 'p_{T} Z [GeV]',               	     ntuplename = pT_Z,  		      bins = 200, minval = 0.0,   maxval = 2000.0,) )
    #
    ...
    #vardb.registerVar( Variable(shortname = 'El0z0sintheta',	   latexname = 'z_{0}*sin(#theta) lead e [mm]',		     ntuplename = 'el_trkz0sintheta[0]',	      bins = 40, minval = -2.0,  maxval = 2.0,) )
    #vardb.registerVar( Variable(shortname = 'El1z0sintheta',	   latexname = 'z_{0}*sin(#theta) 2nd lead e [mm]',	     ntuplename = 'el_trkz0sintheta[1]',	      bins = 40, minval = -2.0,  maxval = 2.0,) )
    ...
    #vardb.registerVar( Variable(shortname = 'Mu0z0sintheta',	   latexname = 'z_{0}*sin(#theta) lead #mu [mm]',            ntuplename = 'muon_trkz0sintheta[0]',	      bins = 40, minval = -2.0,  maxval = 2.0,) )
    #vardb.registerVar( Variable(shortname = 'Mu1z0sintheta',	   latexname = 'z_{0}*sin(#theta) 2nd lead #mu [mm]',	     ntuplename = 'muon_trkz0sintheta[1]',	      bins = 40, minval = -2.0,  maxval = 2.0,) )
...
if doMMClosureRates:
    ...
    # ---> measure fake rate in OS region (requiring at keast one non-prompt)
    #vardb.registerCategory( MyCategory('FakeCRMuL',    cut =  ( vardb.getCuts(['TrigMatch', 'NBJet',       'LowJetCR', 'MuRealFakeRateCR',  'MuProbeAntiTight']) & mu_region & truth_event_cut_F & -vardb.getCut('SS')) ) )  
    #vardb.registerCategory( MyCategory('FakeCRMuT',    cut =  ( vardb.getCuts(['TrigMatch', 'NBJet',       'LowJetCR', 'MuRealFakeRateCR',  'MuProbeTight'    ]) & mu_region & truth_event_cut_F & -vardb.getCut('SS')) ) )  
    #
    ...
    # ---> measure fake rate in OS region (requiring at keast one non-prompt)
    #vardb.registerCategory( MyCategory('FakeCRElL',    cut =  ( vardb.getCuts(['TrigMatch', 'NBJet',       'LowJetCR', 'ElRealFakeRateCR',  'ElProbeAntiTight']) & el_region & truth_event_cut_F & -vardb.getCut('SS') ) ) )  
    #vardb.registerCategory( MyCategory('FakeCRElT',    cut =  ( vardb.getCuts(['TrigMatch', 'NBJet',       'LowJetCR', 'ElRealFakeRateCR',  'ElProbeTight'    ]) & el_region & truth_event_cut_F & -vardb.getCut('SS') ) ) )  
    #
...
if doMMClosureTest:
    print ''   
    #
    # OS/SS dielectron region, no jet cut, no truth requirement
    #
    # NB: if OS region, comment out the TT cut in ttH2015Backgrounds.py for TTBar and manually require 2 tight leptons here, 
    # otherwise you'll see no events 
    #
    #vardb.registerCut( Cut('El1_isTight',  '( el_isTightSelected[0] == 1 )') )
    #vardb.registerCut( Cut('El2_isTight',  '( el_isTightSelected[1] == 1 )') )
    #vardb.registerCategory( MyCategory('ElEl_OS_CR',      cut = ( vardb.getCuts(['NBJet', '2Lep', 'ElEl_Event', 'TauVeto', 'TrigMatch', 'El1_isTight', 'El2_isTight' ]) & -vardb.getCut('SS') ) ) )
    #vardb.registerCategory( MyCategory('ElEl_SS_SR',      cut = vardb.getCuts(['NBJet', '2Lep', 'SS', 'ElEl_Event', 'TauVeto', 'TrigMatch', 'SF_ElEtaCut', 'El1_isTight', 'El2_isTight' ]) ) )
    
    if ( args.fakeMethod == 'MM' or args.fakeMethod == 'FF' ):
        #
        # test region
        #
        #vardb.registerCategory( MyCategory('TestCategory',         cut = vardb.getCuts(['2Lep', 'SS']) ) )
        #
        # MuMu region
        #
        #vardb.registerCategory( MyCategory('MuMuHighJetTrigMatchNoTauTruthMSSBase',         cut = vardb.getCuts(['NBJet', '2Lep', 'SS', 'MuMu_Event', 'TauVeto', 'TrigMatch', 'NJet2L']) ) )
        #vardb.registerCategory( MyCategory('MuMuLowJetTrigMatchNoTauTruthMSSBase',          cut = vardb.getCuts(['NBJet', '2Lep', 'SS', 'MuMu_Event', 'TauVeto', 'TrigMatch', 'LowJetCR']) ) )
        #vardb.registerCategory( MyCategory('MuMuAllJetTrigMatchNoTauTruthMSSBase',          cut = vardb.getCuts(['NBJet', '2Lep', 'SS', 'MuMu_Event', 'TauVeto', 'TrigMatch']) ) )
	...
	# OF region
	#
	#vardb.registerCategory( MyCategory('OFHighJetEtaCutTrigMatchNoTauTruthMSSBase',   cut = vardb.getCuts(['NBJet', '2Lep', 'SS', 'OF_Event', 'TauVeto', 'TrigMatch', 'NJet2L']) ) )   # 'OF_ElEtaCut', 
	#vardb.registerCategory( MyCategory('OFLowJetEtaCutTrigMatchNoTauTruthMSSBase',    cut = vardb.getCuts(['NBJet', '2Lep', 'SS', 'OF_Event', 'TauVeto', 'TrigMatch', 'LowJetCR']) ) ) # 'OF_ElEtaCut', 
	#vardb.registerCategory( MyCategory('OFAllJetEtaCutTrigMatchNoTauTruthMSSBase',    cut = vardb.getCuts(['NBJet', '2Lep', 'SS', 'OF_Event', 'TauVeto', 'TrigMatch']) ) ) # 'OF_ElEtaCut', 
	...
	# ElEl region
	#
	#vardb.registerCategory( MyCategory('ElElHighJetEtaCutTrigMatchNoTauTruthMSSBase',   cut = vardb.getCuts(['NBJet', '2Lep', 'SS', 'ElEl_Event', 'TauVeto', 'TrigMatch', 'NJet2L']) ) )   # 'SF_ElEtaCut', 
	#vardb.registerCategory( MyCategory('ElElLowJetEtaCutTrigMatchNoTauTruthMSSBase',    cut = vardb.getCuts(['NBJet', '2Lep', 'SS', 'ElEl_Event', 'TauVeto', 'TrigMatch', 'LowJetCR']) ) ) # 'SF_ElEtaCut', 
        #vardb.registerCategory( MyCategory('ElElAllJetEtaCutTrigMatchNoTauTruthMSSBase',    cut = vardb.getCuts(['NBJet', '2Lep', 'SS', 'ElEl_Event', 'TauVeto', 'TrigMatch']) ) ) # 'SF_ElEtaCut', 

...
int main( int argc, char* argv[] ) {
   ...
   for( Long64_t entry = 0; entry < entries; ++entry ) {
     ...
     // Tmul the object which entry to look at:
	...
	Info(APP_NAME, "Processed %lld events", entry);
     ...
     if ( debug ) { Info( APP_NAME, "************************* \n number of reco muons: %lu \n *************************", muons->size() ); }
     
     //Iterate over the muons
     for ( auto mu : *(muons) ) { 
     	  
     	  // decorate reconstructed particle with default values
     	  isTruthMatchedDecor( *mu )	    = 0;
     	  isTruthMatchedIsoDecor( *mu )	    = 0;
     	  isTruthMatchedNonIsoDecor( *mu )  = 0;
     	  isTruthMatchedBkgDecor( *mu )	    = 0;
     	  isTruthMatchedOtherDecor( *mu )   = 0;
     	  isTruthMatchedChFlipDecor( *mu )  = 0;
     	  truthMatchedOriginDecor( *mu )    = 0; // i.e., NonDefined  
	  ...
	  if ( debug ) { Info( APP_NAME, "reco muon: " ); }
	  if ( debug ) { Info( APP_NAME, "pT: %f eta: %f phi: %f m: %f ", mu->pt()/1e3 , mu->eta(), mu->phi(), mu->m()/1e3 ); }
          xAOD::IParticle::FourMom_t mu4mom(1., 1., 1., 1.);	
          ( mu4mom  ).SetPtEtaPhiM( mu->pt(), mu->eta(), mu->phi(), mu->m() );
	  ...
	  // loop over truth events
		...
		continue;
	      ...
	      double thisDR = mu4mom.DeltaR( truthPart4mom );
              if ( thisDR < minDR ) { 
          ...
          if ( !matchTruth  ) {
             if ( debug ) { Info( APP_NAME, "No truth matching for this reco muon"); }
             continue;    
          }
	  ...
	  if ( ! matchTruth->isMuon() ) {
	    if ( debug ) { Info( APP_NAME, "This reco muon is truth matched to a NON-muon particle"); }
	    if ( debug ) { Info( APP_NAME, "\t match type: %i - status %i ", matchTruth->pdgId(), matchTruth->status() ); }
	    isTruthMatchedOtherDecor( *mu ) = 1;
	  } else {
	    
	    // okay, this truth match is an muon. Let's see where it comes from...
	    ...
	    if ( matchTruth->hasProdVtx() ) {
		 ...
		 if ( debug ) { Info( APP_NAME, "This reco muon is truth matched to an muon from secondaries"); }
	         if ( debug ) { Info( APP_NAME, "\t parent type: %i - parent status %i ", matchTruth->parent(0)->pdgId(), matchTruth->parent(0)->status() ); }
	         isTruthMatchedBkgDecor( *mu ) = 1;
		...
		// -)  the incoming parton ( status == 2 --> unstable for the generator ) or a photon from ISR/FSR  ( status == 1 --> on shmul photons are stable for the generator ) : PrimaryInteraction(PI) 
		...
		if ( foundHFHAD ) {
		  if ( debug ) { Info( APP_NAME, "Truth muon is non-prompt (HFHAD decay)"); }
		  isTruthMatchedNonIsoDecor( *mu )  = 1;
		} else if ( foundHAD ) {
		  if ( debug ) { Info( APP_NAME, "Truth muon is non-prompt (generic HAD decay)"); }
		  isTruthMatchedNonIsoDecor( *mu )  = 1;
		} else if ( foundPI ) {
		  if ( debug ) { Info( APP_NAME, "Truth muon is prompt"); }
		  isTruthMatchedIsoDecor( *mu )  = 1;
		}
			      
	      }
	      
	    } else {
	      if ( debug ) { Info( APP_NAME, "Truth muon has no production vertex..."); }
	    }

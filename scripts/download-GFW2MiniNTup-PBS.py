...
""" download-GFW2MiniNTup-PBS.py: PBS script to download GFW2 HTop NTuples from EOS """
...
parser = argparse.ArgumentParser(description="PBS NTup skimming submission script for ttHMultilepton. Will submit a single main PBS job made up of an array of subjobs, one for each sample ID.")
...
def copy_source(subdir = string.Template("$TMPDIR").substitute(os.environ), force = False):
    ...
    tarballname = "download_GFW2.tar.gz"
    print("Making a tarball of download scripts and moving it to submission directory...")
    if not os.path.isfile(subdir+"/"+tarballname):
        subprocess.call(["tar","-zcf",tarballname,"ttHMultilepton/scripts/download-GFW2MiniNTup-PBS.py","ttHMultilepton/scripts/wrapper-download-GFW2MiniNTup-PBS.py","--exclude-vcs"])
...
def create_jobs(params):
    ...
    steer_job_name = "download_GFW2.pbs"
...
echo ""
rsync -arvxSH download_GFW2.tar.gz $TMP/
cd $TMP/
echo "Opening tarball..."
echo ""
tar -zxf download_GFW2.tar.gz
rm -rf download_GFW2.tar.gz
echo "Kerberos authentication..."
echo
# NB: the Kerberos credential must be copied in a different directory for each subjob: use TMP!
cp /coepp/cephfs/mel/mmilesi/Kerberos/krb5cc_1016 $TMP
export KRB5CCNAME=$TMP/krb5cc_1016
kinit -R
klist
...
# The following script wraps around the download-GFW2MiniNTup-PBS.py script, picking the sample for the PBS subjob from the input sample list via the env variable os.getenv('PBS_ARRAYID').

# Clear the cache first
ls -l /coepp/cephfs/mel/mmilesi/ttH/

python $PWD/ttHMultilepton/scripts/wrapper-download-GFW2MiniNTup-PBS.py --ntup_tag {ntup_tag} --downloadpath {downloadpath} --samplelist {samples}
...
"410220", # FastSim!
	...
	"subdir"       :  "/coepp/cephfs/mel/mmilesi/ttH/PBS/Download_GFW2MiniNTup/" + prod_ID[0] + "_PBS" + tag,  # The path to the submission node (NB: must NOT be under /home : PBS cannot read/write into it!!)
	"ntup_tag"     :  args.prod_ID,
	"downloadpath" :  "/coepp/cephfs/mel/mmilesi/ttH/GFW2MiniNTup/" + prod_ID[0] + "/" + args.prod_ID + "/gfw2",

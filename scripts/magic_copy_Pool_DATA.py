...
if __name__ == '__main__':
    ...
    for sample in copylist:
        cmd = "mkdir -p 00" + str(sample) + " && cd $_ && xrdcp root://eospublic.cern.ch//eos/escience/UniTexas/HSG8/multileptons_ntuple_run2/" + version + "/" + sample_type + "/00" + str(sample) + ".root . && cd .."
        cmdlist.append(cmd)
        print cmd
    ...
    for chunk in listchunks(cmdlist,MAX_PARALLEL):
        ...
        print("\n".join("{0} - {1}".format(elem[0],elem[1].split()[2]) for elem in enumerate(chunk)))
